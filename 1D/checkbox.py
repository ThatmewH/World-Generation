...
class Checkbox:
    def __init__(self, pos, size,key, text ,active=False, boxOffset=0):
        self.key = key
        self.pos = pos
        self.size = size
    ...
    def update(self,win,mouseDown):
        ...
        if self.rect.collidepoint(mousePos[0]-self.boxOffset, mousePos[1]) and mouseDown and not(self.mouseHeld):
